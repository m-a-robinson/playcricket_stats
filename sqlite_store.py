...
# any junior teams at all (Under 9/Under 9 B/Under 9 D/Under 11/Under 11
...
# list, since every junior team name seen so far states its age group
# plainly; a team that doesn't match stays classed as senior.
_JUNIOR_TEAM_RE = re.compile(
...
def _classify_team(team_name):
    return 1 if team_name and _JUNIOR_TEAM_RE.search(team_name) else 0
...
class SQLiteStore:
    ...
    def _upsert_team(self, source, source_team_id, team_name, club_id):
        ...
        if team_id is None:
            ...
            cursor = self.conn.execute(
                "INSERT INTO teams (team_name, club_id, is_juniors) VALUES (?, ?, ?)",
