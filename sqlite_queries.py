...
appearances AS (
    ...
    WHERE (:season IS NULL OR m.season = :season)
      ...
      AND (:include_youth = 1 OR :team_id IS NOT NULL OR ma.team_id NOT IN (SELECT team_id FROM teams WHERE is_youth = 1))
...
batting_agg AS (
    ...
    WHERE COALESCE(how_out, '') != 'did not bat'
      ...
      AND (:include_youth = 1 OR :team_id IS NOT NULL OR team_id NOT IN (SELECT team_id FROM teams WHERE is_youth = 1))
    GROUP BY player_id
),
bowling_agg AS (
    SELECT
...
fielding_agg AS (
    ...
    WHERE (:season IS NULL OR season = :season)
      ...
      AND (:include_youth = 1 OR :team_id IS NOT NULL OR fielding_team_id NOT IN (SELECT team_id FROM teams WHERE is_youth = 1))
    GROUP BY player_id
)
SELECT
    p.player_id,
...
def career_stats(
    ...
    elpmcc_name=ELPMCC_NAME, include_youth=False
):
    """
    ...
    By default (include_youth=False) appearances/batting/bowling/
    fielding for a youth team (teams.is_youth -- "Under 9", "Under 11",
    see sqlite_store.py's _classify_team()) are excluded from every
    total, the same way opposition players are: youth matches stay in
    the underlying scorecard tables untouched, just not folded into a
    player's main career figures or any leaderboard. Pass
    include_youth=True to include them, or filter to one youth team
    ...
    params = {
        ...
        "include_youth": 1 if include_youth else 0
    }
...
class SQLPlayerStats:
    ...
    def __init__(self, conn, elpmcc_only=True, elpmcc_name=ELPMCC_NAME, include_youth=False):
        self.conn = conn
        self.elpmcc_only = elpmcc_only
        self.elpmcc_name = elpmcc_name
        self.include_youth = include_youth

    # ----------------------------------------------------------

    def career(self, season=None, team_id=None, elpmcc_only=None, include_youth=None):
        return career_stats(
            ...
            include_youth=self.include_youth if include_youth is None else include_youth
        )
